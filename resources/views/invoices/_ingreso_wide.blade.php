{{-- Wide income detail table (formerly the provision-ingresos page), now the
     ingreso tab body. Adds a "show invoice" action alongside Provisión (opens the
     classify modal) and Cobro (opens the Asiento de cobro modal). Expects the
     same $invoices / $q / $perPage the filtered view provides. --}}
<style>
...
<div class="empty-state">
    ...
    <h3>Sin facturas</h3>
    <p>No hay comprobantes de ingreso en el periodo.</p>
</div>
@else
<div class="pi-wrap">
                ...
                <tr data-invoice="{{ $inv->id }}">
                    <td style="white-space:nowrap;">
                        <div class="d-flex align-items-center gap-1">
                            ...
                            @endif
                            {{-- Cobro: ref if generated, else opens cobro modal --}}
                            @if($inv->ingreso_ref)
    ...
    (function () {
        ...
        submit?.addEventListener('click', async () => {
            ...
            await App.loading.button(submit, async () => {
                try {
                    const res = await App.http.post(`${base}/${cobroId}/cobro`, { origen: 'manual', fecha_pago: fecha });
