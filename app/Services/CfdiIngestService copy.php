...
class CfdiIngestService
{
    ...
    private function ingestSingle(string $xml, Client $client, Period $fallbackPeriod): array
    {
        ...
        // Notas de crédito / egresos (TipoDeComprobante "E") are not loaded into
        // the system, per the client's rule. They are reported as "omitidas"
        // rather than stored, so nothing is lost — re-importing after the rule is
        // lifted would bring them in (dedup is by UUID and they were never saved).
        if (($parsed['header']['tipo_comprobante'] ?? null) === 'E') {
            return [
                'status'    => 'omitidas',
                'period_id' => null,
                'reason'    => 'Nota de crédito (Egreso) — no se carga al sistema.',
            ];
        }
